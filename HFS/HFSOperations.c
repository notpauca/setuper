...
static int hfsfuse_read(const char* path, char* buf, size_t size, off_t offset, struct fuse_file_info* info) {
	...
	struct hf_file* f = (struct hf_file*)info->fh;
	uint64_t bytes;
	if (f->is_empty) {return 0; } // empty files have no extents, which to hfslib_readd_with_extents() is an error, so skip it and just return 0 for EOF
